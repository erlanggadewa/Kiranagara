<x-admin-layout>
  <div class="flex flex-col items-center h-[600px]">
    ...
    {{-- Kategori Budaya --}}
    <div class="h-full px-6 py-4 border rounded-lg shadow-lg bg-tertiary border-secondary">
      <form class="h-full " method="POST" enctype="multipart/form-data"
        action="{{ route('kategori-budaya.update', $selectedData->id) }}">
        @csrf
        @method('PUT')
        <input type="hidden" value="kategori" name="template">
        <input type="hidden" value="{{ $selectedData->id }}" name="id">
        <input id="img-name-kategori" type="hidden" value="{{ $selectedData->img }}" name="img">
        <div class="">
          <x-input-basic-update value="{{ $selectedData->name }}" placeholder="Enter Judul Kategori"
            label="Judul Kategori" name="name">
          </x-input-basic-update>
        </div>
        <div class="mt-6">
          <h1 class="mb-2 font-semibold text-gray-600 select-none">Thumbnail</h1>
          <label id="border-drop-area"
            class="relative z-50 flex flex-col w-full p-10 text-center bg-white border-4 rounded-lg cursor-pointer xl:aspect-[3/2] group">
            ...
            <div id="preview-container" class="absolute inset-0 flex items-center justify-center">
              <img id="preview-img-kategori" class="object-cover h-full"
                src="{{ $selectedData->img != 'no-img.png' ? asset('img/budaya/kategori') . '/' . $selectedData->img : asset('img/no-img.png') }}">
            </div>
