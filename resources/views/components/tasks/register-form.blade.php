...
<div {{ $attributes }} class="relative justify-center px-4 py-4 md:flex md:flex-col md:w-1/2 md:px-14">

  <img src="{{ asset('img/pattern2.png') }}" alt="" class="absolute top-0 right-0 w-1/2 md:w-1/3" style="z-index: -999">

  <div x-on:click="openRegister = false" class="cursor-pointer md:hidden mb-7" id="back-login">
  ...
  <h1 class="text-2xl font-semibold text-green-800 ">REGISTER</h1>
  ...
  <div x-cloak x-data>
    <div class="grid grid-cols-2 gap-3">
      {{-- User SVG --}}
      <div x-on:click="$store.userAuth = true" :class="true ? 'border-green-800' : 'border-gray-400'"
    ...
    <div class="grid grid-cols-2 mt-10 mb-8 font-bold">
      ...
      <div class="flex flex-col items-center text-center text-gray-500 ">
        <a href="{{ route('login') }}" class="peer">
          <span class="block duration-200 cursor-pointer hover:scale-105">LOGIN</span>
        </a>
      ...
      <div class="flex flex-col items-center text-center text-green-800 ">
        <a href="{{ route('register') }}">
          <span class="block cursor-pointer">DAFTAR</span>
        </a>
    ...
    <x-auth-validation-error title="Failed to Register"></x-auth-validation-error>

    <template x-if="$store.userAuth">
      <x-tasks.user-register-form></x-tasks.user-register-form>
    </template>

    <template x-if="!$store.userAuth">
      <x-tasks.admin-register-form></x-tasks.admin-register-form>
    </template>
